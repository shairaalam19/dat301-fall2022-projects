...
author: "Amber Kahklen (akahklen, 1216918261), Shaira Alam (salam13, 1216475247), Alyssa Stefencavage (astefenc, 1213852690)"
output: ioslides_presentation
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
df = read.csv("adultdata.csv")
...
knitr::include_graphics('./dataimg.png')
```
...
The lowest count is for people who had a completed doctorate degree. 

The highest category was for grades 1st-4th - which makes sense for education levels where all individuals above the age of 5 will have at least a base level of educaiton. 

## 2. Does the workclass affect if the individual make above or below $50,000 a year?

```{r}

mcsg = length(df$`workclass`[df$`workclass` == " Private" & 
                             df$wage_below_orAbove_50k == " >50K"])
mcsl = length(df$`workclass`[df$`workclass` == " Private" & 
                             df$wage_below_orAbove_50k == " <=50K"])

divg = length(df$`workclass`[df$`workclass` == " Self-emp-not-inc" & 
                             df$wage_below_orAbove_50k == " >50K"])
divl = length(df$`workclass`[df$`workclass` == " Self-emp-not-inc" & 
                             df$wage_below_orAbove_50k == " <=50K"])

nmg = length(df$`workclass`[df$`workclass` == " Self-emp-inc"& 
                             df$wage_below_orAbove_50k == " >50K"])
nml = length(df$`workclass`[df$`workclass` == " Self-emp-inc"& 
                             df$wage_below_orAbove_50k == " <=50K"])

sepg = length(df$`workclass`[df$`workclass` == " Federal-gov"& 
                             df$wage_below_orAbove_50k == " >50K"])
sepl = length(df$`workclass`[df$`workclass` == " Federal-gov"& 
                             df$wage_below_orAbove_50k == " <=50K"])

widg = length(df$`workclass`[df$`workclass` == " Local-gov"& 
                             df$wage_below_orAbove_50k == " >50K"])
widl = length(df$`workclass`[df$`workclass` == " Local-gov"& 
                             df$wage_below_orAbove_50k == " <=50K"])

msag = length(df$`workclass`[df$`workclass` == " State-gov"& 
                             df$wage_below_orAbove_50k == " >50K"])
msal = length(df$`workclass`[df$`workclass` == " State-gov"& 
                             df$wage_below_orAbove_50k == " <=50K"])

masg = length(df$`workclass`[df$`workclass` == " Without-pay"& 
                             df$wage_below_orAbove_50k == " >50K"])
masl = length(df$`workclass`[df$`workclass` == " Without-pay"& 
                             df$wage_below_orAbove_50k == " <=50K"])

nwg = length(df$`workclass`[df$`workclass` == " Never-worked"& 
                             df$wage_below_orAbove_50k == " >50K"])
nwl = length(df$`workclass`[df$`workclass` == " Never-worked"& 
                             df$wage_below_orAbove_50k == " <=50K"])

m_s <- as.table(rbind(c(mcsg, divg, nmg, sepg, widg, msag, masg, nwg), c(mcsl, divl, nml, sepl, widl, msal, masl, nwl)))
dimnames(m_s) <- list(Wage = c(">50K", "<=50K"), Education = c("Private", "Self-emp-not-inc", "Self-emp-inc", "Federal-gov", "Local-gov", "State-gov", "Without-pay", "Never-worked"))
library(knitr)
...
```{r}
par(mar = c(8, 4, 1, 1))
yy <- barplot(m_s, ylim = c(0, 20000), main = "Wage per Workclass", xlab = " ", ylab = "# of people", col = c("orange", "yellow"), names.arg = c("Private", "Self-emp-not-inc", "Self-emp-inc", "Federal-gov", "Local-gov", "State-gov", "Without-pay", "Never-worked"), las = 3, beside=TRUE)
mtext("Workclass", side=1, line = 6)
legend("topright", legend = c("Wage is Below 50k", "Wage is Above 50K"), fill = c("orange", "yellow"))
text(x = yy, y = m_s+700, label=as.character(m_s))
```

## Analysis 

Most individuals in private made over $50,000. 

The next category that leaned heavily with more people making more than $50,000 were those in self-employed not-incorporated. 

However, those who were self-employed and incorporated tended to make less than $50,000. 

## 3. Hours per Week | Wage above or below $50,000 a year?
...
On Average, people that work over 40 hours a week have wages greater than $50,000

## 4. Marital Status | Does making above or below $50,000 have an impact on marital status?
...
There largest group of people that make over $50,000 are married.

The largest group of people that make under $50,000 have never been married.
...
Of all men, 44% of the data has an income over $50,000. 

In comparison, only 12.3% of women have an income over $50,000. 

Based on population of data, this shows a significant discrepency on gender-pay ratios. 
...
Based off the bar graph, people married work the most hours.

Widowed individuals seem to have the lowest hours worked per week. This can be due a variety of reasons, most likely due to increased familial obligations. 
...
Most people with Bachelor's degrees are making less than $50,000 a year while most people with Master's degrees are making more than $50,000 a year.

Most High School graduates make less than $50,000 a year.
...
Farming-fishing had the largest range in hours worked per week. 

Most occupations had a mean that steadied around 40 hours/week. 

## Conclusion

- Education is important to increase pay
- Working at a private organization may provide more pay
- Having a family may impact how many hours someone can work a week
